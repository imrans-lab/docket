...
func test_parser_resolves_registry_after_item_and_preserves_all_json_values() -> Variant:
	...
	if r != true: return r
	r = A.eq(item.fields, {"count":0,"enabled":false,"empty":"","nil":null,"array":[],"object":{},"unicode":"雪","literal":"\\n\\t","actual":"line\nnext\tcell"}, "field envelope preserves every JSON value")
	if r != true: return r
	return A.eq(item.extras.future_payload.nested, [1, true, null], "unknown top-level payload moves into extras")
...
func test_full_export_import_preserves_pins_and_envelopes() -> Variant:
	...
	if r != true: source.close(); target.close(); return r
...
func test_malformed_cached_envelope_cannot_be_flushed_over_canonical_source() -> Variant:
	...
	if r != true: db.close(); return r
	r = A.eq(db.get_item("ORD-0001").fields.count, 0, "cache is reconstructed from canonical source")
	db.close(); return r
...
func test_checked_registry_compound_write_rolls_back_cache_after_canonical_failure() -> Variant:
	...
	if r != true: db._atomic_write_hook = Callable(); db.close(); return r
