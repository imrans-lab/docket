...
`DocketFileIdentity`, which tells which file a project path really is, and
`DocketFileIO`, which writes a new file without ever replacing one and holds
a project's audit lock. It builds with the exact Rust named in
`rust-toolchain.toml`, installed with [rustup](https://rustup.rs), plus the
target for your machine (both Apple
