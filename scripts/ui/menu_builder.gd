...
func _build_file_menu() -> void:
	...
	# Discards the in-memory cache and re-reads the .dct — for after a git pull
	popup.add_item("Reload from Disk", 8)
	popup.set_item_shortcut(popup.get_item_index(8), _make_shortcut(KEY_R, true))
