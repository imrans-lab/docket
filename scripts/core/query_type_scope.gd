...
const UNIVERSAL_FIELDS := ["type", "status", "priority", "severity", "title", "description", "assigned_to", "directed_to", "tags", "has_attachment", "id", "created_at", "updated_at", "project", SelectorQuery.SELECTOR_FIELD, "blocked_by", "parent"]
		...
		for group_condition in group:
			...
			if str(scoped_condition.get("field", "")) not in UNIVERSAL_FIELDS:
				...
				if identities.size() == 1:
					...
					if include_project and not str(single.project).is_empty(): scoped_condition = {"$and":[{"field":SelectorQuery.SELECTOR_FIELD,"op":"eq","value":single.project},scoped_condition]}
					...
					for identity in identities:
						...
						if include_project and not str(record.project).is_empty(): alternative = {"$and":[{"field":SelectorQuery.SELECTOR_FIELD,"op":"eq","value":record.project},alternative]}
...
static func _compile_condition(cond: Dictionary, catalog: Array, include_project: bool) -> Dictionary:
	if cond.get("field", "") == "status" and cond.get("op", "") == "catalog_status":
		...
		if include_project and not str(record.project).is_empty(): predicates.push_front({"field": SelectorQuery.SELECTOR_FIELD, "op": "eq", "value": record.project})
	...
	for key in cond.get("value", []):
		...
		if record.is_empty(): alternatives.append({"field":"type","type_id":str(key),"op":"eq","value":""})
			...
			if include_project and not str(record.project).is_empty(): pair.push_front({"field": SelectorQuery.SELECTOR_FIELD, "op": "eq", "value": record.project})
