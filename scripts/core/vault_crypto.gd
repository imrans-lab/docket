...
## Iteration count for vaults created from now on. OWASP's current guidance for
## PBKDF2-HMAC-SHA256 (Password Storage Cheat Sheet, 2023).
const PBKDF2_ITERATIONS := 600000

## What Docket used before the count was raised. Vaults created then have no
## `vault_kdf_iterations` in their meta, and must keep deriving with this value
## or every existing secret becomes undecryptable. Never change it.
...
static func derive_key(password: String, salt: PackedByteArray, iterations: int = PBKDF2_ITERATIONS) -> PackedByteArray:
	## PBKDF2-HMAC-SHA256 → 32-byte AES key.
	##
	## Always pass the vault's own iteration count (DocketDB.get_vault_iterations)
	## rather than relying on the default: a vault created before the count was
	## raised still derives with LEGACY_PBKDF2_ITERATIONS, and using the wrong
	## count silently produces a wrong key that looks like a wrong password.
	if iterations <= 0:
		iterations = PBKDF2_ITERATIONS
	return _pbkdf2_sha256(password.to_utf8_buffer(), salt, iterations, KEY_LENGTH)
...
static func _pbkdf2_sha256(password: PackedByteArray, salt: PackedByteArray, iterations: int, dk_len: int) -> PackedByteArray:
	...
	##
	## The HMAC key is the password for every one of the `iterations` calls, so
	## the ipad/opad blocks are derived once here rather than rebuilt per call.
	## At the iteration counts modern guidance requires, that padding work would
	...
	var pads := _hmac_pads(password)
	var i_key_pad: PackedByteArray = pads[0]
	var o_key_pad: PackedByteArray = pads[1]
	...
	for block_idx in range(1, blocks_needed + 1):
		...
		var u := _hmac_with_pads(i_key_pad, o_key_pad, salt + int_bytes)
		var result := u.duplicate()

		# U2..Uc
		for _i in range(1, iterations):
			u = _hmac_with_pads(i_key_pad, o_key_pad, u)
...
static func _hmac_pads(key_data: PackedByteArray) -> Array:
	## Returns [i_key_pad, o_key_pad] for HMAC-SHA256 with this key.
	...
	return [i_key_pad, o_key_pad]


static func _hmac_with_pads(i_key_pad: PackedByteArray, o_key_pad: PackedByteArray, data: PackedByteArray) -> PackedByteArray:
	## HMAC-SHA256 given pre-derived pads. Equivalent to _hmac_sha256.
