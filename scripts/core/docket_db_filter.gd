...
##
## SECURITY: field names become SQL identifiers and cannot be parameter-bound —
## SQLite has no placeholder for a column name. Every field must therefore be
## checked against the real columns of the `items` table before interpolation.
## Without that check a caller can supply `title="x" OR 1=1 OR title` as a field
## and rewrite the WHERE clause. DocketDB populates `allowed_fields` from
## PRAGMA table_info at query time, so the set can never drift from the schema.

## Column names accepted as filter/sort fields. Populated by
## DocketDB.execute_query; empty means "not yet initialised", which is treated
## as reject-all rather than allow-all.
static var allowed_fields: Dictionary = {}

## Fields that are not columns but are understood by the translator.
const PSEUDO_FIELDS := ["tags", "has_attachment", "tags_contains"]


static func set_allowed_fields(columns: Array) -> void:
	var d := {}
	for c in columns:
		d[str(c)] = true
	allowed_fields = d
...
static func is_field_allowed(field: String) -> bool:
	if field in PSEUDO_FIELDS:
		return true
	return allowed_fields.has(field)


static func reject_reason(field: String) -> String:
	## "" when usable as a SQL identifier here, else why not.
	if field.is_empty():
		return "empty field name"
	if is_field_allowed(field):
		return ""
	return "unknown query field '%s'" % field
...
static func translate_filter(filter: Dictionary) -> Dictionary:
	...
	for key in filter:
		...
		if key == "tags_contains":
			...
			var why := reject_reason(field)
			if not why.is_empty():
				return {"where": "", "bindings": [], "error": why}
			conditions.append("%s!=?" % field)
			bindings.append(value)
		elif key.ends_with("__in"):
			var field: String = key.substr(0, key.length() - 4)
			var why_in := reject_reason(field)
			if not why_in.is_empty():
				return {"where": "", "bindings": [], "error": why_in}
	...
	for group in groups:
		...
		for cond in group:
			...
			if translated.has("error"):
				return {"where": "", "bindings": [], "error": translated["error"]}
...
static func translate_tree(tree: Dictionary) -> Dictionary:
	if tree.has("$or"):
		...
		for child in tree["$or"]:
			...
			if t.has("error"):
				return {"where": "", "bindings": [], "error": t["error"]}
		...
		if c.has("error"):
			return {"where": "", "bindings": [], "error": c["error"]}
...
static func _condition_to_sql(cond: Dictionary) -> Dictionary:
	...
	# Reject before any interpolation. Callers MUST propagate this rather than
	# skipping the condition: dropping a filter silently widens the result set,
	# which is a worse failure than refusing the query.
	var why := reject_reason(field)
	if not why.is_empty():
		return {"sql": "", "bindings": [], "error": why}
