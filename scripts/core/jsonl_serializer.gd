...
const _EPHEMERAL_META_KEYS := ["jsonl_hash", "jsonl_version", "registry_diagnostics", JSONLCache.UNSAVED_META,
	JSONLCache.PARSE_REVISION_META]
...
static func serialize_meta(db: DocketDB) -> String:
	...
	# derived at. If it were dropped here, a cache rebuild would fall back to the
	# legacy count, derive a different key, and every secret in the file would
	# read as "wrong password" with no way back.
...
static func serialize_secrets(db: DocketDB) -> String:
	...
	for row in secret_rows:
		...
		# owner_item_id: omit when standalone. Must be serialized — a field that
		# lives only in the cache is lost on the next rebuild, which is how the
		# project lifecycle fields disappeared and how the KDF iteration count
		# nearly stranded every vault.
...
static func _serialize_item_row(db: DocketDB, row: Dictionary) -> String:
	...
	if tool_deps is String and not tool_deps.is_empty():
		var parsed_tool_deps = JSON.parse_string(tool_deps)
		if parsed_tool_deps is Array and not parsed_tool_deps.is_empty():
			d["tool_deps"] = parsed_tool_deps
	...
	if optimization is String and not optimization.is_empty():
		var parsed_optimization = JSON.parse_string(optimization)
		if parsed_optimization is Dictionary and not parsed_optimization.is_empty():
			d["optimization"] = parsed_optimization
	...
	# pristine_content: JSON TEXT in SQLite, Dictionary in JSONL.
	var pristine_content = row.get("pristine_content")
	if pristine_content is String and not pristine_content.is_empty():
		var parsed_pristine = JSON.parse_string(pristine_content)
		if parsed_pristine is Dictionary and not parsed_pristine.is_empty():
			d["pristine_content"] = parsed_pristine
	elif pristine_content is Dictionary and not pristine_content.is_empty():
		d["pristine_content"] = pristine_content

	# unsatisfied_deps: JSON Array in SQLite, Array in JSONL.
	var unsatisfied_deps = row.get("unsatisfied_deps")
	if unsatisfied_deps is String and not unsatisfied_deps.is_empty():
		var parsed_unsat = JSON.parse_string(unsatisfied_deps)
		if parsed_unsat is Array and not parsed_unsat.is_empty():
			d["unsatisfied_deps"] = parsed_unsat
