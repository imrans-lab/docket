...
impl Session {
    fn encrypted_session(keypair: &Keypair, session: OpenSessionResult) -> Result<Self, Error> {
        let server_public_key: Vec<u8> = session.output.try_into()?;
        // Docket patch: a key longer than the 1024-bit group's modulus is not
        // a valid public key, and would only cost time to exponentiate.
        if server_public_key.len() > 128 {
            return Err(Error::Crypto("the service's public key is too long"));
        }
